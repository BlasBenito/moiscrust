---
title: 'MOISCRUST: database preparation'
output:
  html_document:
    toc: true
    toc_depth: 2
...
This workflow is packaged with [renv](https://cran.r-project.org/package=renv) to facilitate reproducibility. To run this interactive notebook in your machine, please, execute first the code chunk below. You will need to replace `eval = FALSE` with `eval = TRUE` in the header of the code chunk.
...
for(i in 1:nrow(sensors_pairs)){
  ...
  #common cases
  sensors_pairs[i, "valid_cases_shared_percent"] <- nrow(sensor_pair_i) / nrow(moiscrust) * 100
...
## Applying gap filling to the whole dataset
...
moiscrust_long[is.na(moiscrust_long$interpolation_quality), "interpolation_quality"] <- "observation"
